...
<template>
    <div class="col-md-12 posts">
        <p v-if="!weathers.length">No city selected</p>
        <div v-if="weathers.length > 0" class="col-md-12 mt-3">
            <h2 href="#">{{ city.city }}</h2>
            <p>Average temperature in the next 10 days: </p>
            <p class="averagetemp">{{ average }}&#176;</p>
        </div>
        <div v-if="weathers.length > 0">
          <h3 class="mr-3" href="#">The next 10 days</h3>
          <hr/>
        </div>
        <div class="row">
          <div class="col-md-1">&nbsp;</div>
          <div class="col-md-1 weatherfullbox" v-for="weather in weathers" :key="weathers.id">
            <p>{{new Date(weather.datetime).toLocaleDateString('en-US', {day: 'numeric',month: 'short',})}}</p>
            <div v-if="weather.weather == 'hot'" class="weatherbox hot">
              &nbsp;
              <span class='tempvalue'>{{ weather.temp }}&#176;</span>
              <span class="sun"></span>
              <span class="sunx"></span>
            </div>
            <div v-if="weather.weather == 'cloudy'" class="weatherbox cloudy">
              &nbsp;
              <span class='tempvalue'>{{ weather.temp }}&#176;</span>
            	<span class="cloud"></span>
            	<span class="cloudx"></span>
            </div>
            <div v-if="weather.weather == 'stormy'" class="weatherbox stormy">
              &nbsp;
              <span class='tempvalue'>{{ weather.temp }}&#176;</span>
            	<ul>
            	<li></li>
            ...
            <div v-if="weather.weather == 'breezy'" class="weatherbox breezy">
              &nbsp;
            	<ul>
...
export default {
    ...
    mounted() {
        Event.$on('found_country', (data, average, city) => {
